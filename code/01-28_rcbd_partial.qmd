...
#test question: uses everything before it in pipe for next function :)
rcbd_dfw <- rcbd_df %>% # w for wrangle :) 
...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~krate_kgha) + #before or after "~" shows whether plots will be grouped by factor vertically (by rows), or horizontally (by columns)
  theme(legend.position = "none")
```
...
# Model fitting
rcbd_mod <- lm(yield_kgha ~ rep + nrate_kgha*krate_kgha, #rep is what makes this a RCBD, otherwise its  a CRD
               data = rcbd_dfw)
...
```{r rcbd final plot}
#raw data AND pairwise comparisons
ggplot() +  
