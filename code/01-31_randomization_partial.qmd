...
library(viridis) # for changing fill scale
```
...
ggplot(data = crd_layout, aes(x = row, y = -col)) +
  geom_tile(aes(fill = trtname), color = "black", show.legend = FALSE) + 
  geom_text(aes(label = lab), size = 2) + 
  labs(title = "CRD randomization of a 3x3 factorial (9 treatments)") + 
  scale_fill_viridis_d() +
  theme_void()

ggsave("../2025_dsa/04_rcbd/output/crd_layoutmap.png",
       width = 6,
       height = 4,
       bg = "white")
...
ggplot(data = rcbd_layout, aes(x = row, 
  ...
  geom_text(aes(label = plot), fontface = "bold", size = 2, vjust = -.8)+
  geom_text(aes(label = trtname), size = 2)+
...
rcbd_rand %>%
  ...
  write_csv("../2025_dsa/04_rcbd/output/rcbd_randomization_table.csv")
```
...
```{r}
ggsave("../2025_dsa/04_rcbd/output/rcbd_layoutmap.png",
       width = 6,
       height = 4,
       bg = "white")
```
